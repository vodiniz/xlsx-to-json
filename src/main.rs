...
impl fmt::Debug for PokemonTrade {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(
            ...
            "Pokemon: {}\n
        Pokemon to trade: {}\n",
            self.wanted_pokemon, self.pokemon
        )
    }
}
...
impl fmt::Debug for PokemonGift {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(
            ...
            "Pokemon: {}
            Gift Requirements: {}\n",
            self.pokemon, self.gift_requirement
        )
    }
}
        ...
        if self.obs.len() > 0 {
            write!(
                ...
                "\nPokemon: {}\n
                    Price: {}\n
                    Observation: {}\n",
                self.pokemon, self.price, self.obs
            )
        } else {
            write!(
                f,
...
impl fmt::Debug for UnobtainablePokemon {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
...
impl fmt::Debug for Encounters<UnobtainablePokemon> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        // Use `self.number` to refer to each positional data point.
        let mut tmp_string = String::new();
...
fn get_separate_encounters(
    day_vec: Vec<Pokemon>,
    night_vec: Vec<Pokemon>,
) -> (Vec<Pokemon>, Vec<Pokemon>, Vec<Pokemon>) {
    let day_set: HashSet<Pokemon> = HashSet::from_iter(day_vec);
    let night_set = HashSet::from_iter(night_vec);

    let day_only_set = day_set
        .difference(&night_set)
        .cloned()
        .collect::<Vec<Pokemon>>();

    let night_only_set = night_set
        .difference(&day_set)
        .cloned()
        .collect::<Vec<Pokemon>>();

    let always_set = night_set
        .intersection(&day_set)
        .cloned()
        .collect::<Vec<Pokemon>>();
...
enum EncountersType {
    Grass(Option<Encounters<RouteInfo<EncounterTime>>>),
    Water(Option<Encounters<RouteInfo<EncounterTimeWater>>>),
    Trade(Encounters<RouteInfo<PokemonTrade>>),
    Gift(Encounters<RouteInfo<PokemonGift>>),
    Egg(Encounters<RouteInfo<EggEncounter>>),
    Fossil(Encounters<RouteInfo<FossilEncounter>>),
    Unobtaible(Encounters<UnobtainablePokemon>),
}

fn create_pokemon_location_json() {
    let encounters_vec: Vec<EncountersType> = vec![
        EncountersType::Grass(calamine_grass_extractor()),
        EncountersType::Water(calamine_water_extractor()),
        EncountersType::Trade(calamine_trade_pokemon_extractor()),
        EncountersType::Gift(calamine_gift_pokemon_extractor()),
        EncountersType::Egg(calamine_egg_vendor_extractor()),
        EncountersType::Fossil(calamine_fossil_extractor()),
        EncountersType::Unobtaible(calamine_unobtainable_pokemon_extractor()),
    ];

    let all_encounters: AllEncounters<EncountersType> = AllEncounters {
        encounters: encounters_vec,
    };

    let f = File::create("pokemon_encounters.json").expect("Unable to create file");
    let bw = BufWriter::new(f);
    serde_json::to_writer(bw, &all_encounters).expect("Failed writing :(");
}

fn get_pokemon_list() -> Vec<Pokemonname> {
    let mut json_file = std::fs::File::open("all_pokemon.json").expect("Can't open json");
    let mut buff = String::new();
    json_file.read_to_string(&mut buff).unwrap();
    let json: Pokemonlist = serde_json::from_str(&buff).expect("Couldn't Serialize Json");
...
fn calamine_grass_extractor() -> Option<Encounters<RouteInfo<EncounterTime>>> {
    let mut excel: Xlsx<_> =
        open_workbook("pokemon_locations.xlsx").expect("Couldn't open pokemon xlsx");
    ...
    if let Some(Ok(document)) = excel.worksheet_range(&sheets[0]) {
        ...
        loop {
            ...
            for (pokemon_column, rate_column) in (document
                .range((2, column_number), (13, column_number))
                .cells())
            .zip(document.range((2, 0), (13, 0)).used_cells())
            {
                let pokemon_name = pokemon_column
                    .2
                    .get_string()
                    .expect("Failed to read Pokemon name");

                let pokemon_encounterrate = rate_column
                    .2
                    .get_float()
                    .expect("Failed to read Pokemon encounter rate");

                let current_pokemon: Pokemon = Pokemon {
                    pokemon: replace_galarian_alola(pokemon_name),
                    encounterrate: pokemon_encounterrate,
                };

                if day_pokemon_vector.contains(&current_pokemon) {
                    let index = day_pokemon_vector
                        .iter()
                        .position(|r| r == &current_pokemon)
                        .expect("Can't find pokemon index.");
                    day_pokemon_vector[index].encounterrate =
                        (day_pokemon_vector[index].encounterrate) * 2.0;
                } else {
            ...
            for (pokemon_column, rate_column) in (document
                .range((16, column_number), (27, column_number))
                .cells())
            .zip(document.range((2, 0), (13, 0)).used_cells())
            {
                let pokemon_name = pokemon_column
                ...
                if night_pokemon_vector.contains(&current_pokemon) {
                    let index = night_pokemon_vector
                        .iter()
                        .position(|r| r == &current_pokemon)
                        .expect("Can't find pokemon index.");
                    night_pokemon_vector[index].encounterrate =
                        (night_pokemon_vector[index].encounterrate) * 2.0;
                } else {
                    night_pokemon_vector.push(current_pokemon);
                }
            }

            let (only_day, only_night, always) =
                get_separate_encounters(day_pokemon_vector, night_pokemon_vector);
            let encounter_time: EncounterTime = EncounterTime {
                always: always,
                day: only_day,
                night: only_night,
            };
            let route_info: RouteInfo<EncounterTime> = RouteInfo {
                route: route_name.to_string(),
                encounter_info: encounter_time,
            };
        ...
        let encounters: Encounters<RouteInfo<EncounterTime>> = Encounters {
            encountermethod: "Grass".to_string(),
            routes: route_vector,
        };
...
fn get_route_name(
    column_number: u32,
    document: &calamine::Range<calamine::DataType>,
) -> Option<String> {
...
fn calamine_water_extractor() -> Option<Encounters<RouteInfo<EncounterTimeWater>>> {
    ...
    if let Some(Ok(document)) = excel.worksheet_range(&sheets[1]) {
        ...
        let encounters: Encounters<RouteInfo<EncounterTimeWater>> = Encounters {
            encountermethod: "Fishing and Surfing".to_string(),
            routes: water_encounters,
        };
        Some(encounters)
    } else {
        None
...
fn get_rods_surf_list(
    route_name: String,
    column_number: u32,
    document: &calamine::Range<calamine::DataType>,
) -> RouteInfo<EncounterTimeWater> {
...
fn get_old_rod(column_number: u32, document: &calamine::Range<calamine::DataType>) -> Vec<Pokemon> {
    ...
    for (pokemon_column, rate_column) in (document
        .range((2, column_number), (3, column_number))
        .cells())
    .zip(document.range((2, 0), (3, 0)).used_cells())
    {
        let tuple = (pokemon_column.2.get_string(), rate_column.2.get_float());
        match tuple {
            (Some(string), Some(float)) => {
                pokemon_vec = check_pokemon_in_vec(
                    pokemon_vec,
                    Pokemon {
                        pokemon: replace_galarian_alola(string),
                        encounterrate: float as f64,
                    },
                )
            }
            _ => (),
...
fn get_good_rod(
    ...
    let mut pokemon_vec = vec![];
    for (pokemon_column, rate_column) in (document
        .range((5, column_number), (7, column_number))
        .cells())
    .zip(document.range((5, 0), (7, 0)).used_cells())
    {
        let tuple = (pokemon_column.2.get_string(), rate_column.2.get_float());
        match tuple {
            (Some(string), Some(float)) => {
...
fn get_super_rod(
    ...
    let mut pokemon_vec = vec![];
    for (pokemon_column, rate_column) in (document
        .range((9, column_number), (13, column_number))
        .cells())
    .zip(document.range((9, 0), (13, 0)).used_cells())
    {
        let tuple = (pokemon_column.2.get_string(), rate_column.2.get_float());
        match tuple {
            (Some(string), Some(float)) => {
...
fn get_surfing(column_number: u32, document: &calamine::Range<calamine::DataType>) -> Vec<Pokemon> {
    let mut pokemon_vec = vec![];
    for (pokemon_column, rate_column) in (document
        .range((15, column_number), (19, column_number))
        .cells())
    .zip(document.range((15, 0), (19, 0)).used_cells())
    {
        let tuple = (pokemon_column.2.get_string(), rate_column.2.get_float());
        match tuple {
            (Some(string), Some(float)) => {
...
fn make_route_info(
    ...
    let route_info: RouteInfo<EncounterTimeWater> = RouteInfo {
        route: route_name,
        encounter_info: encounter_water,
    };
    route_info
}

fn get_trade_pokemon(
    row_number: u32,
    document: &calamine::Range<calamine::DataType>,
    ...
    let tuple = (
        range[0][0].get_string(),
        range[0][1].get_string(),
        range[0][2].get_string(),
    );
...
fn calamine_trade_pokemon_extractor() -> Encounters<RouteInfo<PokemonTrade>> {
    ...
    if let Some(Ok(document)) = excel.worksheet_range(&sheets[2]) {
        ...
        loop {
            match get_trade_pokemon(row_number, &document) {
                Some(tuple) => {
                    let trade_pokemon: PokemonTrade = PokemonTrade {
                        pokemon: replace_galarian_alola(&tuple.1),
                        wanted_pokemon: replace_galarian_alola(&tuple.2),
                    };
                    let route_info: RouteInfo<PokemonTrade> = RouteInfo {
                        route: tuple.0,
                        encounter_info: trade_pokemon,
                    };
    ...
    let encounters: Encounters<RouteInfo<PokemonTrade>> = Encounters {
        encountermethod: "Trade".to_string(),
        routes: route_vec,
    };
    encounters
}
...
fn calamine_gift_pokemon_extractor() -> Encounters<RouteInfo<PokemonGift>> {
    ...
    if let Some(Ok(document)) = excel.worksheet_range(&sheets[3]) {
        ...
        loop {
            match get_trade_pokemon(row_number, &document) {
                Some(tuple) => {
                    let trade_pokemon: PokemonGift = PokemonGift {
                        pokemon: replace_galarian_alola(&tuple.0),
                        gift_requirement: replace_galarian_alola(&tuple.2),
                    };
                    let route_info: RouteInfo<PokemonGift> = RouteInfo {
                        route: tuple.1,
                        encounter_info: trade_pokemon,
                    };
                    route_vec.push(route_info);
    ...
    let encounters: Encounters<RouteInfo<PokemonGift>> = Encounters {
        encountermethod: "Gift".to_string(),
        routes: route_vec,
    };
    encounters
}

fn get_egg_vendor_pokemon(
    document: &calamine::Range<calamine::DataType>,
) -> Option<(Vec<EggVendor>, Vec<EggVendor>)> {
    ...
    while column_number < 3 {
        ...
        match shard_type {
            Some(shard_str) => {
                for pokemon_cell in document
                    .range((3, column_number), (8, column_number))
                    .used_cells()
                {
                    if pokemon_cell.0 > 3 {
                        break;
                    ...
                    match pokemon_cell.2.get_string() {
                        Some(pokemon) => {
                            let pokemon_egg: EggVendor = EggVendor {
                                pokemon: replace_galarian_alola(pokemon),
                                price: format!("1 {}", shard_str),
                            };
    ...
    for pokemon_cell in document.range((2, 5), (15, 5)).used_cells() {
        match pokemon_cell.2.get_string() {
            Some(pokemon) => {
                let pokemon_egg: EggVendor = EggVendor {
                    pokemon: replace_galarian_alola(pokemon),
                    price: "$100 000 each, additional $100 000 for Shiny form.".to_string(),
                };
...
fn calamine_egg_vendor_extractor() -> Encounters<RouteInfo<EggEncounter>> {
    ...
    if let Some(Ok(document)) = excel.worksheet_range(&sheets[4]) {
        match get_egg_vendor_pokemon(&document) {
            Some(tuple) => {
                let egg_encounter: EggEncounter = EggEncounter {
                    mansion: tuple.0,
                    gamecorner: tuple.1,
                };
                let route_info_mansion: RouteInfo<EggEncounter> = RouteInfo {
                    route: "Celadon".to_string(),
                    encounter_info: egg_encounter,
                };
    ...
    let encounters: Encounters<RouteInfo<EggEncounter>> = Encounters {
        encountermethod: "Fossil".to_string(),
        routes: route_vec,
    };
    encounters
}
...
fn get_fossil_pokemon(document: &calamine::Range<calamine::DataType>) -> FossilEncounter {
    ...
    while column_number < 4 {
        ...
        match shard_type {
            Some(shard_str) => {
                for pokemon_cell in document
                    .range((3, column_number), (5, column_number))
                    .used_cells()
                {
                    match pokemon_cell.2.get_string() {
                        Some(pokemon) => {
                            let pokemon_split: Vec<_> =
                                pokemon.split([' ', ','].as_ref()).collect();

                            let pokemon_fossil: Fossil = Fossil {
                                pokemon: replace_galarian_alola(pokemon_split[0]),
                                price: format!(
                                    "{} {}s",
                                    pokemon_split[1].replace("(", ""),
                                    shard_str.replace(":", "")
                                ),
                                obs: "".to_string(),
                            };
    ...
    let fossil_encounter: FossilEncounter = FossilEncounter {
        trade: trade_fossil_list,
        found: found_fossil_list,
    };
    fossil_encounter
}
...
fn calamine_fossil_extractor() -> Encounters<RouteInfo<FossilEncounter>> {
    ...
    if let Some(Ok(document)) = excel.worksheet_range(&sheets[5]) {
        ...
        let route_info: RouteInfo<FossilEncounter> = RouteInfo {
            route: "Mansion".to_string(),
            encounter_info: fossil_encounter,
        };
        route_vec.push(route_info);
    }
    let encounters: Encounters<RouteInfo<FossilEncounter>> = Encounters {
        encountermethod: "Egg Vendor".to_string(),
        routes: route_vec,
    };
    encounters
}
...
fn get_unobtainable_pokemon(
    document: &calamine::Range<calamine::DataType>,
) -> Vec<UnobtainablePokemon> {
    let mut unobtaible_vec = vec![];
    for pokemon_cell in document.range((1, 0), (9, 3)).used_cells() {
        unobtaible_vec.push(UnobtainablePokemon {
            pokemon: pokemon_cell
                .2
                .get_string()
                .expect("No valid name")
                .to_string(),
        })
    }
    unobtaible_vec
...
fn calamine_unobtainable_pokemon_extractor() -> Encounters<UnobtainablePokemon> {
    let mut excel: Xlsx<_> =
        open_workbook("pokemon_locations.xlsx").expect("Couldn't open pokemon xlsx");
    let sheets = excel.sheet_names().to_owned();
    let mut unobtainable_vec = vec![];
    if let Some(Ok(document)) = excel.worksheet_range(&sheets[6]) {
        unobtainable_vec = get_unobtainable_pokemon(&document);
    }
    let encounters: Encounters<UnobtainablePokemon> = Encounters {
        encountermethod: "Unobtainable".to_string(),
        routes: unobtainable_vec,
    };
    encounters
}

fn main() {
    let _list = get_pokemon_list();
    create_pokemon_location_json();
}
